...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      ...
      if (fileList.some(value => !value.upload)) {
        this.$message.warning('图片还在上传,请稍等')
        return
      }
      const staffPhoto = fileList.length ? fileList[0].url : ' '
      await saveUserDetailById({ ...this.userInfo, staffPhoto: staffPhoto })
      this.$message.success('保存成功')
    },
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto?.trim()) {
        ...
        this.$refs.pic.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    }
  }
}
